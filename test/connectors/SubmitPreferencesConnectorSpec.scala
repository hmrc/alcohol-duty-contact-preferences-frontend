...
class SubmitPreferencesConnectorSpec extends SpecBase {

  "submitContactPreferences must" - {
    val mockUrl = s"http://alcohol-duty-contact-preferences/submit-preferences/$appaId"
    ...
    "successfully submit contact preferences" in new SetUp {
      ...
      when(mockConfig.ecpSubmitContactPreferencesUrl(eqTo(appaId))).thenReturn(mockUrl)
